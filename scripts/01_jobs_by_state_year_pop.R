...
pop_2010_2020 <- read.csv("C:\\Users\\12898\\Desktop\\Trefler\\data\\pop_2010_2020.csv")
pop_2020_2024 <- read.csv("C:\\Users\\12898\\Desktop\\Trefler\\data\\pop_2020_2024.csv")
...
jobs_by_year_state <- ds %>%
  ...
  mutate(
    post_date = as.Date(post_date),   # Arrow-supported cast
    year = year(post_date)
  ) %>%
  group_by(year, state) %>%
...
to_long <- function(df) {
  df %>%
    ...
    mutate(
      year = as.integer(sub("POPESTIMATE", "", year)),
      population = as.numeric(population)
    )
}

pop_long_all <- bind_rows(
  to_long(pop_2010_2020),
...
jobs_pop <- jobs_by_year_state %>%
  ...
  mutate(
    jobs_per_cap = n_jobs / population
  )
...
jobs_2023 <- jobs_pop %>%
  mutate(
    ...
    state = if_else(state %in% c("Washington, D.C.", "Washington, D.C. "),
...
p_2023 <- ggplot(map_df_2023) +
  geom_sf(aes(fill = log_jobs_per_10k), color = "white", linewidth = 0.2) +
  scale_fill_viridis_c(option = "mako", na.value = "grey92") +
  labs(
    title = "Job postings per 18+ by state (2023)",
    fill  = element_blank()
  ) +
  coord_sf(datum = NA) +
...
p_facetted <- ggplot(map_df_all) +
  geom_sf(aes(fill = jobs_pct), color = "white", linewidth = 0.2) +
  ...
  labs(
    title = "Job postings intensity per capita by state (within-year percentile)",
    fill  = "Percentile"
  ) +
  coord_sf(datum = NA) +
  theme_void(base_size = 12) +
